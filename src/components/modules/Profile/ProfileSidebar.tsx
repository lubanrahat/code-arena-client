...
export default function ProfileSidebar({ user, stats }: ProfileSidebarProps) {
  const joinDate = user?.createdAt ? format(new Date(user.createdAt), "MMM yyyy") : "Unknown";
  const { profile } = user || {};
        ...
        <CardContent className="p-6 flex flex-col items-start gap-4">
          ...
          <div className="w-full space-y-3 mt-2">
            ...
            <div className="flex items-center gap-2 text-sm text-muted-foreground">
              ...
              <span className="truncate">{profile?.institution || "Not Set"}</span>
            </div>
        ...
        <CardContent className="p-5 flex flex-col gap-4">
          ...
          <div className="space-y-4 pt-2">
            <div className="flex justify-between items-center text-sm border-b border-border/50 pb-2">
              ...
              <span className="font-medium text-foreground">{stats?.globalRank?.toLocaleString() || "---"}</span>
            </div>
            <div className="flex justify-between items-center text-sm border-b border-border/50 pb-2">
              <span className="text-muted-foreground">University Rank</span>
              <span className="font-medium text-foreground">{stats?.universityRank?.toLocaleString() || "---"}</span>
            </div>
            <div className="flex justify-between items-center text-sm border-b border-border/50 pb-2">
              <span className="text-muted-foreground">Time Spent</span>
              <span className="font-medium text-foreground">{stats?.timeSpent || "0 hours"}</span>
            </div>
            <div className="flex justify-between items-center text-sm">
              <span className="text-muted-foreground">Total Problems Solved</span>
              <span className="font-medium text-foreground">{stats?.totalProblemsSolved || 0}</span>
            </div>
          </div>
        </CardContent>
      </Card>
